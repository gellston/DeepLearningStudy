...
optimizer = torch.optim.Adam(CSPSeparableResnet18CenterNet.parameters(), lr=learning_rate, amsgrad=True)
...
for epoch in range(training_epochs): # 앞서 training_epochs의 값은 15로 지정함.
    ...
    CSPSeparableResnet18CenterNet.eval()
    CSPSeparableResnet18.eval()
    compiled_model_backbone = torch.jit.script(CSPSeparableResnet18)
    torch.jit.save(compiled_model_backbone, "C://Github//DeepLearningStudy//trained_model//TRAIN_WIDERFACE(CSPSeparableResnet18CenterNetBackBone).pt")
    compiled_model_head = torch.jit.script(CSPSeparableResnet18CenterNet)
    torch.jit.save(compiled_model_head, "C://Github//DeepLearningStudy//trained_model//TRAIN_WIDERFACE(CSPSeparableResnet18CenterNet).pt")
    ## no Train Model Save
    print('학습중간에 저장')
