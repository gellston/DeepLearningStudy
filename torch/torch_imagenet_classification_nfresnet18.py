...
## no Train Model Save
if pretrained == True:
    CSPMobileNetV2Weight = torch.jit.load("D://Github//DeepLearningStudy//trained_model//ImageNet(NFResNet18).pt")
    model.load_state_dict(CSPMobileNetV2Weight.state_dict())

model.eval()
compiled_model = torch.jit.script(model)
torch.jit.save(compiled_model, "D://Github//DeepLearningStudy//trained_model//ImageNet(NFResNet18).pt")
## no Train Model Save
...
for epoch in range(current_epoch, training_epochs): # 앞서 training_epochs의 값은 15로 지정함.
    ...
    for X, Y in validation_data_loader:
        ...
        current_batch += 1
        print('current validation batch=', current_batch, '/', total_validation_batch, ' accuracy=', accuracy.item(), ' cost=', cost.item())
    ...
    plt.show(block=False)
    plt.pause(0.001)
    plt.savefig('D://Github//DeepLearningStudy//trained_model//ImageNet(NFResNet18).png')
