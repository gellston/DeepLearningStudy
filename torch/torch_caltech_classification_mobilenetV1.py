...
model = MobileNetV1(class_num=257, activation=torch.nn.SiLU).to(device)
print('==== model info ====')
...
torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//CALTECH256(MobileNetV1).pt")

trace_input = torch.rand(1, 3, 512, 512).to(device, dtype=torch.float32)
trace_model = torch.jit.trace(model, trace_input)
torch.jit.save(trace_model, "C://Github//DeepLearningStudy//trained_model//CALTECH256(MobileNetV1)_Trace.pt")
...
model.eval()
compiled_model = torch.jit.script(model)
torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//TRAIN_CALTECH256(MobileNetV1).pt")
## no Train Model Save
