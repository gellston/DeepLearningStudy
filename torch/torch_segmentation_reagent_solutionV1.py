...
from model.SolSegV1 import SolSegV1
...
SolSegV1 = SolSegV1(class_num=classNum,
...
print('==== model info ====')
summary(SolSegV1, (1, 512, 128))
SolSegV1.eval()
compiled_model = torch.jit.script(SolSegV1)
torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//SolSegV1_no_train.pt")
print('====================')
...
optimizer = torch.optim.RAdam(SolSegV1.parameters(), lr=learning_rate)
...
for epoch in range(training_epochs):
    ...
    SolSegV1.eval()
    compiled_model = torch.jit.script(SolSegV1)
    torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//SolSegV1_step.pt")
    ...
    if top_accuracy <= average_accuracy:
        ...
        SolSegV1.eval()
        compiled_model = torch.jit.script(SolSegV1)
        torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//SolSegV1_top.pt")

    if average_accuracy >= target_accuracy:
        SolSegV1.eval()
        compiled_model = torch.jit.script(SolSegV1)
        torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//SolSegV1_final.pt")
        break
