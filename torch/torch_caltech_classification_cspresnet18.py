...
torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//CALTECH256(CSPResnet18).pt")

trace_input = torch.rand(1, 3, 512, 512).to(device, dtype=torch.float32)
trace_model = torch.jit.trace(model, trace_input)
torch.jit.save(trace_model, "C://Github//DeepLearningStudy//trained_model//CALTECH256(CSPResnet18)_Trace.pt")
...
model.eval()
compiled_model = torch.jit.script(model)
torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//TRAIN_CALTECH256(CSPResnet18).pt")
## no Train Model Save
