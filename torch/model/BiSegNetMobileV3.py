...
class BiSegNetMobileV3(torch.nn.Module):
    ...
    def __init__(self, class_num=5, activation=torch.nn.ReLU6):
        ...
        self.layer1 = torch.nn.Sequential(
            InvertedBottleNeck(in_channels=8, out_channels=16, expansion_rate=3, stride=2, activation=activation),
            InvertedBottleNeck(in_channels=16, out_channels=16, expansion_rate=3, stride=1, activation=activation)
        ) #4

        self.layer2 = torch.nn.Sequential(
            InvertedBottleNeck(in_channels=16, out_channels=24, expansion_rate=3, stride=2, activation=activation),
            InvertedBottleNeck(in_channels=24, out_channels=24, expansion_rate=3, stride=1, activation=activation)
        ) #8

        self.layer3 = torch.nn.Sequential(
            InvertedBottleNeck(in_channels=24, out_channels=32, expansion_rate=3, stride=2, activation=activation),
            InvertedBottleNeck(in_channels=32, out_channels=32, expansion_rate=3, stride=1, activation=activation),
            InvertedBottleNeck(in_channels=32, out_channels=32, expansion_rate=3, stride=1, activation=activation)
        ) #16
