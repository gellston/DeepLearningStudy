...
model = MobileNetV3Small(class_num=4).to(device)
print('==== model info ====')
...
torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//FIAT(MobileNetV3Small).pt")

trace_input = torch.rand(1, 3, 224, 224).to(device, dtype=torch.float32)
trace_model = torch.jit.trace(model, trace_input)
torch.jit.save(trace_model, "C://Github//DeepLearningStudy//trained_model//FIAT(MobileNetV3Small)_Trace.pt")
...
model.eval()
compiled_model = torch.jit.script(model)
torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//TRAIN_FIAT(MobileNetV3Small).pt")
## no Train Model Save
