...
from model.PerceptronSimpleMnist import PerceptronSimpleMnist
...
model = PerceptronSimpleMnist().to(device)
...
for epoch in range(training_epochs):  # 앞서 training_epochs의 값은 15로 지정함.
    avg_cost = 0
    avg_acc = 0
    total_batch = len(data_loader) #전체 데이터셋 갯수 / 배치사이즈(100)
    ...
    for X, Y in data_loader:
        ...
        cost.backward() #back propagation 동작, 코스트 함수의 기울기
        optimizer.step() #optimizer로 한스탭 내려감 (경사하강해버렸음)

        ##acc calculation
        model.eval()
        prediction = model(X)
        correct_prediction = torch.argmax(prediction, 1) == Y
        accuracy = correct_prediction.float().mean() # Batch사이즈 정확도
