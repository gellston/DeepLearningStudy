...
model = CSPMobileNetV2(class_num=257, activation=torch.nn.ReLU6).to(device)
print('==== model info ====')
...
torch.jit.save(compiled_model, "C://Github//DeepLearningStudy//trained_model//CALTECH256(CSPMobileNetV2).pt")

trace_input = torch.rand(1, 3, 640, 640).to(device, dtype=torch.float32)
trace_model = torch.jit.trace(model, trace_input)
torch.jit.save(trace_model, "C://Github//DeepLearningStudy//trained_model//CALTECH256(CSPMobileNetV2)_Trace.pt")
