...
class Linear(tf.keras.layers.Layer):
  def __init__(self, units=32, input_dim=32):
    ...
    self.w = tf.Variable(initial_value=self.w_init_value(shape=(input_dim, units), dtype="float32"),trainable=True,)
    self.b = tf.Variable(initial_value=self.b_init_value(shape=(units,), dtype="float32"), trainable=True)
