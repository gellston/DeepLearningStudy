...
from model.OnyxSegmentation import OnyxSegmentation
...
loader_train = opendl_segmentation_dataloader('C://Users//gellston//Desktop//FinalOnyxAugmentation//')
...
model = OnyxSegmentation(learning_rate=learning_rate)
tf.keras.utils.plot_model(model.get_model(), to_file='C:\\Github\\DeepLearningStudy\\trained_model\\OnyxSegmentation.png', show_shapes=True, show_layer_names=True)
for epoch in range(500):
    average_cost = 0
    average_accuracy = 0
    for batch in range(total_batch):
        inputs_train, outputs_train = loader_train.load([512, 512, 1], [512, 512, 1], 1, 255, batch_size)
    ...
    loader_test.clear()
    loader_test.shuffle()
    input_test, output_test = loader_test.load([512, 512, 1], [512, 512, 1], 1, 255, 1)
...
tf.saved_model.save(model.get_model(), "C:\\Github\\DeepLearningStudy\\trained_model\\OnyxSegmentation")

# Save the weights
model.get_model().save_weights("C:\\Github\\DeepLearningStudy\\trained_model\\OnyxSegmentation.h5")

# Save the model architecture as json
with open("C:\\Github\\DeepLearningStudy\\trained_model\\OnyxSegmentation.json", "w") as fp:
